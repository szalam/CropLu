...
df = read.csv('data-for-plotting4.csv')
head(df)
colnames(df) = c('Crop' ,'Year'   ,'Acreage' ,'Yield' ,'Price','Price_infl')

df$Acreage = df$Acreage/1000/2.4711
df$Price = df$Price_infl#/0.907185
df$Yield = df$Yield #* 2.4711*0.907185
summary(df)

pval =read.csv('Pvalue.csv')
r2error = read.csv('R2error.csv')
...
lm_eqn <- function(df){
  ...
  if(coef(m)[2]<0){
    if(coef(m)[1]>=0){
      eq <- substitute(italic(y) == a ~'\u2013'~ b %*% italic(year), 
...
lm_eqn2 <- function(df, s, p){
  m <- lm(y ~ x, df);
  eq <- substitute(italic(R)^2~"="~r2 ~'\u00b1'~ s ~","~ italic("p"[MK])~"="~p, 
...
for(i in 1:length(crop.uniq)){
  ...
  p = pval[pval$ï..crop==crop.uniq[i],3]
  s = r2error[r2error$ï..crop==crop.uniq[i],3]
  ...
  p2 = ggplot(data=df.tmp, aes(x=Year, y=Yield)) +
    ...
    ylim(min(df.bt$y),(max(df.bt$y,na.rm = T)*1.05))+
    theme(axis.text.x=element_blank())+
    theme(axis.title.x = element_blank(),
          axis.title.y = element_blank())
  
  
  max_min_diff = (max(df.bt$y,na.rm = T) - min(df.bt$y))/8
  p2=p2 + geom_text(x = 2011, y = (max(df.bt$y,na.rm = T)*1.04), label = lm_eqn(df.bt), parse = TRUE,size = 2.8)+
    geom_text(x = 2011, y = (max(df.bt$y,na.rm = T)*1.04-max_min_diff), label = lm_eqn2(df.bt,s=s,p=p), parse = TRUE,size = 2.5)
  
  if(max(df.bt$y,na.rm = T)>=0 & max(df.bt$y)<10){
    ggsave(p2,filename=paste0(wd$figure,"yield_cropwise/Yield_series_",crop.uniq[i],".png"),
           width =5, height = 4, units = "cm")}
  if(max(df.bt$y,na.rm = T)>10 & max(df.bt$y)<100){
    ggsave(p2,filename=paste0(wd$figure,"yield_cropwise/Yield_series_",crop.uniq[i],".png"),
           width =5, height = 4, units = "cm")}
  if(max(df.bt$y,na.rm = T)>=100 & max(df.bt$y)<1000){
    ggsave(p2,filename=paste0(wd$figure,"yield_cropwise/Yield_series_",crop.uniq[i],".png"),
           width =5.06, height = 4, units = "cm")}
  if(max(df.bt$y,na.rm = T)>=1000 & max(df.bt$y)<10000){
    ggsave(p2,filename=paste0(wd$figure,"yield_cropwise/Yield_series_",crop.uniq[i],".png"),
           width =5.24, height = 4, units = "cm")}
  if(max(df.bt$y,na.rm = T)>=10000 & max(df.bt$y)<100000){
    ggsave(p2,filename=paste0(wd$figure,"yield_cropwise/Yield_series_",crop.uniq[i],".png"),
...
for(i in 1:length(crop.uniq)){
  ...
  p = pval[pval$ï..crop==crop.uniq[i],4]
  s = r2error[r2error$ï..crop==crop.uniq[i],4]
  ...
  if(crop.uniq[i]=="Cotton Pima"){
    p2=p2 + geom_text(x = 2011, y = (max(df.bt$y,na.rm=T)*1), label = lm_eqn(df.bt), parse = TRUE,size = 2.8)+
      geom_text(x = 2011, y = (max(df.bt$y,na.rm=T)*1-max_min_diff), label = lm_eqn2(df.bt,s=s,p=p), parse = TRUE,size = 2.8)
  }else{
  p2=p2 + geom_text(x = 2011, y = (max(df.bt$y,na.rm=T)*1.04), label = lm_eqn(df.bt), parse = TRUE,size = 2.8)+
    geom_text(x = 2011, y = (max(df.bt$y,na.rm=T)*1.04-max_min_diff), label = lm_eqn2(df.bt,s=s,p=p), parse = TRUE,size = 2.5)
  }
...
for(i in 1:length(crop.uniq)){
  ...
  p = pval[pval$ï..crop==crop.uniq[i],2]
  s = r2error[r2error$ï..crop==crop.uniq[i],2]
  ...
  p2=p2 + geom_text(x = 2011, y = (max(df.bt$y,na.rm = T)*1.04), label = lm_eqn(df.tmp), parse = TRUE,size = 2.8)+
    geom_text(x = 2011, y = (max(df.bt$y,na.rm = T)*1.04-max_min_diff), label = lm_eqn2(df.tmp,s=s,p=p), parse = TRUE,size = 2.5)
